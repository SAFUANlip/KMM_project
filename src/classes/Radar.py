...
from config.constants import GuidedMissile_SPEED, MAX_DIST_ERROR
...
class RadarRound(Simulated):
    ...
    def find_objects(self):
        ...
        for obj in all_objects:
            r = np.linalg.norm(obj._pos - self._pos) #FIXME: pos? x y z?
            if r < self.view_distance:
                ...
                if self.pan_cur < pan < self.pan_cur + self.pan_per_sec and self.tilt_cur < tilt < self.tilt_cur + self.tilt_per_sec:
                    pos = obj._pos + np.random.randint(-MAX_DIST_ERROR, MAX_DIST_ERROR, size=3)
                    speed_direction = obj.vel + np.random.randint(-0.001 * GuidedMissile_SPEED, 0.001 * GuidedMissile_SPEED, size=3)
